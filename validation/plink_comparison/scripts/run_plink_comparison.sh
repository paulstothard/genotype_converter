...
usage() {
  cat <<'EOF'
Run a PLINK conversion with this converter and compare the output BIM to an expected BIM.

Usage:
  validation/plink_comparison/scripts/run_plink_comparison.sh [options]

Required:
  --original-bfile PREFIX   Original PLINK fileset prefix.
  --expected-bfile PREFIX   Expected converted PLINK fileset prefix.
  --out-prefix PREFIX       Output prefix for this converter.
  --report-prefix PREFIX    Report prefix for comparison CSV/Markdown.
  --from-format FORMAT      Original allele format.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --original-bfile) ORIGINAL_BFILE="$2"; shift 2 ;;
    --expected-bfile) EXPECTED_BFILE="$2"; shift 2 ;;
...
for value_name in ORIGINAL_BFILE EXPECTED_BFILE OUT_PREFIX REPORT_PREFIX FROM_FORMAT; do
